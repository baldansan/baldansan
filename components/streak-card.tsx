...
export function StreakCard({ summary, showDailyReview = true }: Props) {
  const progressPercent = Math.round(
    (Math.min(summary.today.total, summary.dailyGoal) / summary.dailyGoal) * 100
  );
    ...
    <section className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-700 p-5 text-white shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 flex items-baseline gap-2">
            ...
            <span className="text-sm text-emerald-100">
              {summary.currentStreak === 1 ? "өдөр" : "өдөр"}
            </span>
          </p>
        ...
        <div className="rounded-xl bg-white/15 px-4 py-3 text-right ring-1 ring-white/20">
          ...
          <p className="mt-1 text-2xl font-bold">
            {Math.min(summary.today.total, summary.dailyGoal)}/{summary.dailyGoal}
          </p>
          <p className="text-xs text-emerald-100">
            {summary.goalMet ? "✓ биелсэн" : "үргэлжлүүл"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-5">
        ...
        <p className="mt-2 text-xs text-emerald-50">
          Хичээл {summary.today.lessonEvents} · Үг {summary.today.wordEvents} · Quiz{" "}
          {summary.today.quizEvents}
        </p>
      </div>
      ...
      {showDailyReview ? (
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          >
            {summary.goalMet ? "Daily review" : "Өнөөдрийн review"}
          </Link>
          {!summary.goalMet ? (
            <Link
              href="/courses/hsk5"
