...
export function GameWordSourcePicker({
  ...
  return (
    <div className="bs-game-source">
      <h2 className="bs-mem-step-title">Юугаар тоглох вэ?</h2>
          ...
          return (
            ...
            >
              ...
              <span className="bs-game-source-body">
                <span className="bs-game-source-title">{opt.title}</span>
                <span className="bs-game-source-desc">
                  {disabled ? "Нэвтэрсний дараа идэвхжинэ" : opt.desc}
                </span>
              </span>
            </button>
          );
      ...
      {poolNote ? (
        <p className="mt-3 text-xs font-semibold text-amber-700">{poolNote}</p>
      ) : null}
    </div>
  );
}
