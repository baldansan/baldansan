...
type Props = {
  ...
  onUpdate: (
    id: string,
    patch: Partial<Pick<LaunchCheckItem, "status" | "notes">>
  ) => void;
};
...
export function LaunchChecklist({ section, items, onUpdate }: Props) {
          ...
          >
            ...
            <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center">
              <select
                ...
                onChange={(e) =>
                  onUpdate(item.id, {
                    status: e.target.value as LaunchCheckStatus,
                  })
                }
...
export function LaunchBlockersList({ items }: { items: LaunchCheckItem[] }) {
